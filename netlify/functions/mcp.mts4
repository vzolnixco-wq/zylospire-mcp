...
> = {
  ...
  call_external_api_example: {
    ...
    handler: async (args) => {
      const apiKey = Deno.env.get("sk-3iEFIJIS6AHk3WNPsFS2cUN3dhHnO0DW4dUTCQzgKdfDsEpY");
      if (!apiKey) {
        return {
          content: [
            {
      ...
      // Replace this URL/body with the real API you want to call.
      ...
      return {
        content: [
          {
            type: "text",
            text: `(demo) Would call external API with query: "${args.query}"`,
          },
        ],
      };
    },
